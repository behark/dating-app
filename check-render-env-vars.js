...
Object.keys(envVarsInRender).forEach(key => {
  ...
  if (config.value === 'AUTO_GENERATED') {
    console.log(`  ✅ ${key}: AUTO_GENERATED by Render`);
  } else {
...
Object.keys(criticalVars).forEach(key => {
  if (envVarsInRender[key]) {
    console.log(`  ✅ ${key}: ${envVarsInRender[key].value === 'AUTO_GENERATED' ? 'AUTO_GENERATED' : 'SET'}`);
  } else {
    console.log(`  ❌ ${key}: MISSING - ${criticalVars[key]}`);
  }
});
console.log();
...
Object.keys(importantVars).forEach(key => {
  if (envVarsInRender[key]) {
    ...
    console.log(`  ⚠️  ${key}: MISSING - ${importantVars[key]}`);
  }
});
console.log();
...
console.log(`  Critical variables set: ${Object.keys(criticalVars).length - criticalMissing.length}/${Object.keys(criticalVars).length}`);
console.log(`  Important variables set: ${Object.keys(importantVars).length - importantMissing.length}/${Object.keys(importantVars).length}`);
console.log();
