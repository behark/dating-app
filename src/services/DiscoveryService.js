...
class DiscoveryService {
  ...
  async exploreUsers(lat, lng, options = {}) {
    try {
      ...
      // #region agent log
      fetch('http://127.0.0.1:7242/ingest/052d01ac-3f86-4688-97f8-e0e7268e5f14',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({location:'DiscoveryService.js:47',message:'exploreUsers options parsed',data:{guest,hasGuestOption:'guest' in options,optionsKeys:Object.keys(options)},timestamp:Date.now(),sessionId:'debug-session',runId:'run2',hypothesisId:'F'})}).catch(()=>{});
      // #endregion
      ...
      if (guest) {
        ...
        // #region agent log
        fetch('http://127.0.0.1:7242/ingest/052d01ac-3f86-4688-97f8-e0e7268e5f14',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({location:'DiscoveryService.js:72',message:'Guest parameter added to query',data:{finalUrl:`/discovery/explore?${queryParams.toString()}`},timestamp:Date.now(),sessionId:'debug-session',runId:'run2',hypothesisId:'F'})}).catch(()=>{});
        // #endregion
      } else {
        // #region agent log
        fetch('http://127.0.0.1:7242/ingest/052d01ac-3f86-4688-97f8-e0e7268e5f14',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({location:'DiscoveryService.js:76',message:'Guest parameter NOT added',data:{guest,queryString:queryParams.toString()},timestamp:Date.now(),sessionId:'debug-session',runId:'run2',hypothesisId:'F'})}).catch(()=>{});
        // #endregion
      }
