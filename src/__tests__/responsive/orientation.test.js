...
describe('Orientation Handling', () => {
  ...
  describe('Landscape Orientation', () => {
    ...
    it('should handle iPhone landscape', () => {
      jest.spyOn(Dimensions, 'get').mockReturnValue({ width: 667, height: 375 });
      const { result } = renderHook(() => useResponsive());
      expect(result.current.isLandscape).toBe(true);
  ...
  describe('Edge Cases', () => {
    ...
    it('should handle very wide screens', () => {
      jest.spyOn(Dimensions, 'get').mockReturnValue({ width: 2560, height: 1080 });
      const { result } = renderHook(() => useResponsive());
      expect(result.current.isLandscape).toBe(true);
    });

    it('should handle very tall screens', () => {
      jest.spyOn(Dimensions, 'get').mockReturnValue({ width: 1080, height: 2560 });
      const { result } = renderHook(() => useResponsive());
      expect(result.current.isLandscape).toBe(false);
    });
  });
