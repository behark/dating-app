#!/usr/bin/env node

/**
 * Fetch Render environment variables via API
 */

const https = require('https');

const SERVICE_ID = 'srv-d5cooc2li9vc73ct9j70';
const API_KEY = process.env.RENDER_API_KEY || 'rnd_uxGa5DLMWLzFvyvRlvhxslstAyaO';

function makeRequest(path) {
  return new Promise((resolve, reject) => {
    const options = {
      hostname: 'api.render.com',
      path: path,
      method: 'GET',
      headers: {
        'Authorization': `Bearer ${API_KEY}`,
        'Accept': 'application/json',
      },
    };

    const req = https.request(options, (res) => {
      let data = '';

      res.on('data', (chunk) => {
        data += chunk;
      });

      res.on('end', () => {
        try {
          const json = JSON.parse(data);
          resolve({ status: res.statusCode, data: json });
        } catch (e) {
          resolve({ status: res.statusCode, data: data });
        }
      });
    });

    req.on('error', (error) => {
      reject(error);
    });

    req.end();
  });
}

async function fetchEnvVars() {
  console.log('='.repeat(80));
  console.log('Fetching Render Environment Variables via API');
  console.log('='.repeat(80));
  console.log('');

  try {
    // Try the env-vars endpoint
    console.log('Fetching environment variables...');
    const response = await makeRequest(`/v1/services/${SERVICE_ID}/env-vars`);
    
    console.log(`Status: ${response.status}`);
    console.log('');

    if (response.status === 200 && response.data) {
      if (Array.isArray(response.data)) {
        console.log(`Found ${response.data.length} environment variables:\n`);
        console.log('-'.repeat(80));
        
        const envVars = {};
        response.data.forEach(item => {
          const env = item.envVar || item;
          if (env && env.key) {
            // Get the value
            let value = '[NOT SET]';
            if (env.value !== undefined && env.value !== null && env.value !== '') {
              // Mask sensitive values
              if (env.key.includes('SECRET') || env.key.includes('KEY') || env.key.includes('PASSWORD') || env.key.includes('TOKEN')) {
                value = env.value.length > 20 ? env.value.substring(0, 20) + '...' : '***';
              } else if (env.key === 'MONGODB_URL' || env.key === 'MONGODB_URI') {
                // Mask MongoDB connection string
                const match = env.value.match(/mongodb\+srv:\/\/([^:]+):([^@]+)@/);
                if (match) {
                  value = `mongodb+srv://${match[1]}:***@${env.value.split('@')[1]}`;
                } else {
                  value = env.value.substring(0, 30) + '...';
                }
              } else {
                value = env.value;
              }
            } else if (env.generateValue === true) {
              value = '[AUTO-GENERATED BY RENDER]';
            }
            
            envVars[env.key] = { value: env.value, display: value };
            console.log(`${env.key.padEnd(30)} = ${value}`);
          }
        });
        
        console.log('');
        console.log('-'.repeat(80));
        console.log('');
        
        // Check critical variables
        console.log('📋 CRITICAL VARIABLES STATUS:');
        console.log('-'.repeat(80));
        
        const critical = {
          'MONGODB_URI': 'Required - MongoDB connection string',
          'JWT_SECRET': 'Required - JWT signing secret',
          'ENCRYPTION_KEY': 'Required - Data encryption key',
          'CORS_ORIGIN': 'Required - Frontend URL',
          'NODE_ENV': 'Required - Environment',
          'PORT': 'Required - Server port',
        };
        
        Object.keys(critical).forEach(key => {
          // Check for both MONGODB_URI and MONGODB_URL
          const checkKey = key === 'MONGODB_URI' ? (envVars['MONGODB_URI'] || envVars['MONGODB_URL']) : envVars[key];
          
          if (checkKey) {
            const status = checkKey.display.includes('NOT SET') ? '❌ MISSING' : 
                          checkKey.display.includes('AUTO-GENERATED') ? '✅ AUTO-GENERATED' : '✅ SET';
            console.log(`  ${status.padEnd(20)} ${key.padEnd(25)} ${critical[key]}`);
            if (key === 'MONGODB_URI' && envVars['MONGODB_URL']) {
              console.log(`  ⚠️  NOTE: Variable is named MONGODB_URL (not MONGODB_URI)`);
            }
          } else {
            console.log(`  ❌ MISSING           ${key.padEnd(25)} ${critical[key]}`);
          }
        });
        
        console.log('');
        console.log('-'.repeat(80));
        console.log('');
        
        // Check important variables
        console.log('📋 IMPORTANT VARIABLES STATUS:');
        console.log('-'.repeat(80));
        
        const important = [
          'REDIS_HOST', 'REDIS_URL', 'FIREBASE_PRIVATE_KEY', 'FIREBASE_CLIENT_EMAIL',
          'STORAGE_PROVIDER', 'CLOUDINARY_CLOUD_NAME', 'CLOUDINARY_API_KEY',
          'CLOUDINARY_API_SECRET', 'STRIPE_SECRET_KEY', 'GOOGLE_CLIENT_ID',
          'GOOGLE_CLIENT_SECRET'
        ];
        
        important.forEach(key => {
          if (envVars[key]) {
            const status = envVars[key].display.includes('NOT SET') ? '⚠️  MISSING' : '✅ SET';
            console.log(`  ${status.padEnd(20)} ${key}`);
          } else {
            console.log(`  ⚠️  MISSING          ${key}`);
          }
        });
        
      } else {
        console.log('Response format:', JSON.stringify(response.data, null, 2));
      }
    } else {
      console.log('Response:', JSON.stringify(response.data, null, 2));
    }

    console.log('');
    console.log('='.repeat(80));

  } catch (error) {
    console.error('❌ Error:', error.message);
  }
}

fetchEnvVars();
