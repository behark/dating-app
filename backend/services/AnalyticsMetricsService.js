...
class AnalyticsMetricsService {
  ...
  async getSwipeToMatchConversion(startDate, endDate = new Date()) {
    ...
    const conversionRate = totalSwipes > 0 
      ? (totalMatches / totalSwipes) * 100 
      : 0;
  ...
  async getMessageResponseRate(startDate, endDate = new Date()) {
    ...
    const responseRate = totalFirstMessages > 0 
      ? (respondedMessages / totalFirstMessages) * 100 
      : 0;
  ...
  async getPremiumChurnRate(startDate, endDate = new Date()) {
    ...
    const churnRate = startPremium > 0 
      ? (cancelledInPeriod / startPremium) * 100 
      : 0;
  ...
  async getRetentionRate(cohortDate, retentionDays = [1, 7, 30]) {
    ...
    for (const days of retentionDays) {
      ...
      datadogService.gauge(`retention.D${days}`, rate, [`cohort:${cohortStart.toISOString().split('T')[0]}`]);
    }
  ...
  async getRollingRetention(lookbackDays = 30) {
    ...
    const retentionRate = eligibleUsers > 0 
      ? (eligibleActiveCount / eligibleUsers) * 100 
      : 0;
