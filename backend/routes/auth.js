...
router.post('/google', [
  ...
  body('email').isEmail().normalizeEmail()
], handleValidationErrors, googleAuth);

router.post('/facebook', [
  body('facebookId').notEmpty().withMessage('Facebook ID is required'),
  body('email').isEmail().normalizeEmail()
], handleValidationErrors, facebookAuth);

router.post('/apple', [
  body('appleId').notEmpty().withMessage('Apple ID is required')
], handleValidationErrors, appleAuth);

// Phone verification routes
router.post('/send-phone-verification', authenticate, [
  body('phoneNumber').notEmpty().withMessage('Phone number is required')
], handleValidationErrors, sendPhoneVerification);
